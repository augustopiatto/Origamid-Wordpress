...
function api_transacao_post($request) {
    ...
    $user_id = $user->ID;
    $produto_vendido = $request['produto']['venedido'] === 'false';
    ...
    if($user_id > 0) {
        ...
        //: Serve para o campo no banco virar um json, ao invés de ter quer criar um meta
        //: field para cada valor dentro do objeto
        $endereco = json_encode($request['endereco'], JSON UNESCAPED_UNICODE);
        $produto = json_encode($request['produto'], JSON UNESCAPED_UNICODE);
